...
inline long ScoreLocal(long scorej,
                       long leni, long lenj,
...
//------------------------------------------------------------ DiffAligns ------
inline long DiffAligns(const DeltaEdgelet_t *i, const DeltaEdgelet_t *j)
{
  return ((j->loR < i->loR) ? labs(j->hiR - i->loR) : labs(i->hiR - j->loR)) +
         ((j->loQ < i->loQ) ? labs(j->hiQ - i->loQ) : labs(i->hiQ - j->loQ));
}

//------------------------------------------------------------ PickBest --------
long PickBest(const LIS_t *lis, const vector<long> &allbest, float epsilon)
{
...
//------------------------------------------------------------ ScoreLocal ------
inline long ScoreLocal(long scorej, long leni, long lenj,
                       long olap, float idyi, float maxolap)
{
  if (olap > 0 &&
...
//----------------------------------------------------------- ScoreGlobal ------
inline long ScoreGlobal(long scorej, long leni, long olap, float idyi)
{
  return (scorej + (long)((leni - olap) * pow(idyi, 2)));
}
...
//------------------------------------------------------------ UpdateBest ------
bool UpdateBest(LIS_t *lis, long size, vector<long> &allbest, float epsilon)
{
  if (size == 0)
    return false;
      ...
      (best == size ||
       ...
       (epsilon >= 0 &&
        (float)(lis[allbest.front()].score - lis[best].score) /
                (float)(lis[allbest.front()].score) * 100.0 >
            epsilon)))
    return false;
...
bool DeltaAlignment_t::read(std::istream &is, const bool promer, const bool read_deltas)
{
  ...
  do
  {
    ...
    if (delta < 0)
      total++; // delta < 0 means there is a deletion in the reference
    if (read_deltas)
...
//===================================================== DeltaReader_t ==========
//----------------------------------------------------- open -------------------
void DeltaReader_t::open(const string &delta_path)
{
...
void DeltaGraph_t::build(const string &deltapath, bool getdeltas)
{
  ...
  while (dr.readNext(getdeltas))
  {
    ...
    insret = refnodes.insert(map<string, DeltaNode_t>::value_type(dr.getRecord().idR, DeltaNode_t()));
    ...
    insret = qrynodes.insert(map<string, DeltaNode_t>::value_type(dr.getRecord().idQ, DeltaNode_t()));
      ...
      do
      {
        ...
        for (i = 0; i < n; ++i)
        {
          ...
          for (j = 0; j < i; ++j)
          {
            ...
            score = ScoreGlobal(lis[j].score, len, olap, lis[i].a->idy);
    ...
    do
    {
      ...
      for (i = 0; i < n; ++i)
      {
        ...
        for (j = 0; j < i; ++j)
        {
          ...
          score = ScoreLocal(lis[j].score, leni, lenj, olap, lis[i].a->idy, maxolap);
